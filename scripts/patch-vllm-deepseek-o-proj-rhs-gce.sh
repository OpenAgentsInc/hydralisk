...
OUTPUT_DIR="${OUTPUT_DIR:-$PWD/.hydralisk/deepseek-v4-o-proj-rhs-patch-$TS}"
...
render_evidence() {
  local md="$OUTPUT_DIR/o-proj-rhs-patch.md"
  {
    echo "# DeepSeek-V4 o_proj grouped RHS patch"
    echo
    ...
    if [[ "$DRY_RUN" = "1" ]]; then
      ...
      echo
      echo '```json'
      sed -n '1,120p' "$OUTPUT_DIR/o-proj-rhs-patch.jsonl" 2>/dev/null || true
      echo '```'
      echo
      echo "## Stderr"
      echo
      echo '```text'
      sed -n '1,120p' "$OUTPUT_DIR/o-proj-rhs-patch.stderr" 2>/dev/null || true
      echo '```'
    fi
    echo
    echo "## Public safety"
...
REQUIRED_RECIPE_MARKER = "Hydralisk issue #10 DeepSeek o_proj recipe probe"
MARKER = "Hydralisk issue #11 DeepSeek o_proj grouped RHS probe"
...
NEW_CALL = '''    rhs_weight = wo_a.weight
    ...
    if os.environ.get("HYDRALISK_DEEPSEEK_O_PROJ_GROUP_RHS", "0") == "1":
        # Hydralisk issue #11 DeepSeek o_proj grouped RHS probe.
...
def load_target():
    ...
    backup = path.with_name(path.name + ".hydralisk-o-proj-rhs.bak")
    return path, backup
...
def main():
    ...
    recipe_patch_present = REQUIRED_RECIPE_MARKER in text

    base = {
        "schema": "hydralisk.deepseek-v4.o-proj-rhs-patch.v1",
        "action": action,
        "path": str(path),
        "backup": str(backup),
        "backupExists": backup.exists(),
        "patched": patched,
        "recipePatchPresent": recipe_patch_present,
    ...
    if not recipe_patch_present:
        base.update({"ok": False, "error": "o_proj_recipe_patch_required"})
        emit(base)
        return 3

    if OLD_CALL not in text:
        base.update({"ok": False, "error": "expected_call_block_not_found"})
        emit(base)
        return 3
...
$remote_patch
PY" \
  > "$OUTPUT_DIR/o-proj-rhs-patch.jsonl" \
  2> "$OUTPUT_DIR/o-proj-rhs-patch.stderr"
