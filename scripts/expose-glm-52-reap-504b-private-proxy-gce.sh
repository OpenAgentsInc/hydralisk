...
EVIDENCE_REF="${EVIDENCE_REF:-docs/evidence/2026-06-24-glm-52-reap-504b-load-smoke.md}"
...
run_gcloud compute ssh "$TARGET_INSTANCE" \
  ...
  --zone "$TARGET_ZONE" \
  --quiet \
  --command="ACTION='$ACTION' RUN_ID='$RUN_ID' BUNDLE_PATH='/tmp/hydralisk-glm52-proxy-$RUN_ID.tar.gz' REMOTE_SRC_DIR='$REMOTE_SRC_DIR' REMOTE_VENV_DIR='$REMOTE_VENV_DIR' REMOTE_STATE_DIR='$REMOTE_STATE_DIR' REMOTE_LOG_DIR='$REMOTE_LOG_DIR' PROXY_HOST='$PROXY_HOST' PROXY_PORT='$PROXY_PORT' UPSTREAM_BASE_URL='$UPSTREAM_BASE_URL' SERVED_MODEL_NAME='$SERVED_MODEL_NAME' MODEL_ALIASES='$MODEL_ALIASES' MODEL_REVISION='$MODEL_REVISION' ENGINE_VERSION='$ENGINE_VERSION' CONTAINER_IMAGE='$CONTAINER_IMAGE' MODEL_PROFILE_REF='$MODEL_PROFILE_REF' ADMISSION_REF='$ADMISSION_REF' EVIDENCE_REF='$EVIDENCE_REF' DEFAULT_MIN_P='$DEFAULT_MIN_P' DEFAULT_REPETITION_PENALTY='$DEFAULT_REPETITION_PENALTY' DEFAULT_MAX_TOKENS='$DEFAULT_MAX_TOKENS' DEFAULT_ENABLE_THINKING='$DEFAULT_ENABLE_THINKING' MAX_INFLIGHT_REQUESTS='$MAX_INFLIGHT_REQUESTS' TOKEN_FILE='$REMOTE_STATE_DIR/bearer-token' bash /tmp/hydralisk-glm52-proxy-$RUN_ID.sh"
