...
REPLICA_REF="${REPLICA_REF:-glm52-reap-primary-g4-tp4}"
REPLICA_PROFILE_REF="${REPLICA_PROFILE_REF:-glm-reap-504b-g4-tp4-mtp2-rp105}"
REPLICA_DRAINING="${REPLICA_DRAINING:-false}"
REPLICA_RESERVED="${REPLICA_RESERVED:-false}"
REPLICA_RESERVATION_REF="${REPLICA_RESERVATION_REF:-}"
PROVISIONING_CLASS="${PROVISIONING_CLASS:-spot}"
MAX_RUN_DURATION_PRESENT="${MAX_RUN_DURATION_PRESENT:-false}"
WATCHDOG_REF="${WATCHDOG_REF:-hydralisk-glm52-reap-watchdog-5m}"
WATCHDOG_STATUS="${WATCHDOG_STATUS:-configured}"
WATCHDOG_CHECKED_AT="${WATCHDOG_CHECKED_AT:-}"
KEEPWARM_STATUS_PATH="${KEEPWARM_STATUS_PATH:-/var/log/hydralisk/glm52-reap-keepwarm/latest-public.json}"
...
REPLICA_RESERVED=$REPLICA_RESERVED
REPLICA_RESERVATION_REF=$REPLICA_RESERVATION_REF
PROVISIONING_CLASS=$PROVISIONING_CLASS
MAX_RUN_DURATION_PRESENT=$MAX_RUN_DURATION_PRESENT
WATCHDOG_REF=$WATCHDOG_REF
WATCHDOG_STATUS=$WATCHDOG_STATUS
WATCHDOG_CHECKED_AT=$WATCHDOG_CHECKED_AT
KEEPWARM_STATUS_PATH=$KEEPWARM_STATUS_PATH
AUTH_REQUIRED=true
PUBLIC_BIND=false
...
export HYDRALISK_REPLICA_REF="$REPLICA_REF"
export HYDRALISK_REPLICA_PROFILE_REF="$REPLICA_PROFILE_REF"
export HYDRALISK_REPLICA_DRAINING="$REPLICA_DRAINING"
export HYDRALISK_REPLICA_RESERVED="$REPLICA_RESERVED"
export HYDRALISK_REPLICA_RESERVATION_REF="$REPLICA_RESERVATION_REF"
export HYDRALISK_PROVISIONING_CLASS="$PROVISIONING_CLASS"
export HYDRALISK_MAX_RUN_DURATION_PRESENT="$MAX_RUN_DURATION_PRESENT"
export HYDRALISK_WATCHDOG_REF="$WATCHDOG_REF"
export HYDRALISK_WATCHDOG_STATUS="$WATCHDOG_STATUS"
export HYDRALISK_WATCHDOG_CHECKED_AT="$WATCHDOG_CHECKED_AT"
export HYDRALISK_KEEPWARM_STATUS_PATH="$KEEPWARM_STATUS_PATH"
...
start_action() {
  ...
  HYDRALISK_REPLICA_REF="$REPLICA_REF" \
  HYDRALISK_REPLICA_PROFILE_REF="$REPLICA_PROFILE_REF" \
  HYDRALISK_REPLICA_DRAINING="$REPLICA_DRAINING" \
  HYDRALISK_REPLICA_RESERVED="$REPLICA_RESERVED" \
  HYDRALISK_REPLICA_RESERVATION_REF="$REPLICA_RESERVATION_REF" \
  HYDRALISK_PROVISIONING_CLASS="$PROVISIONING_CLASS" \
  HYDRALISK_MAX_RUN_DURATION_PRESENT="$MAX_RUN_DURATION_PRESENT" \
  HYDRALISK_WATCHDOG_REF="$WATCHDOG_REF" \
  HYDRALISK_WATCHDOG_STATUS="$WATCHDOG_STATUS" \
  HYDRALISK_WATCHDOG_CHECKED_AT="$WATCHDOG_CHECKED_AT" \
  HYDRALISK_KEEPWARM_STATUS_PATH="$KEEPWARM_STATUS_PATH" \
